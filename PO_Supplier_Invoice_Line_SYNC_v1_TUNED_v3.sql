/* ============================================================================
   Workday EIB – PO Invoice Lines – tuned (NO LOGIC CHANGE)
   - Same business rules as PO_Supplier_Invoice_Line_SYNC_v1.sql
   - Performance: restrict SERVICE_FLAGS + PO_HAS_POS_AMT_AND_QTY to PO candidates only
   - Performance: remove unused recv_agg join (no SUOM dependency)
   ============================================================================ */
...
/* Header candidates (same filters; add lookback early to reduce work — no output change because included_po enforces it anyway) */
...
/* Matched+Paid vouchered amount/qty per PO line/sched (as-of) */
vchr_sum_match AS (
  SELECT
...
/* PO line flags (needed for sched_open) */
po_line_flags AS (
  SELECT /*+ MATERIALIZE */
      l.business_unit,
      l.po_id,
...
/* Open schedule logic (same as v1; uses qty_vchr and $1 tolerance) */
sched_open AS (
  SELECT
      x.business_unit,
      x.po_id,
      ...
      CASE
        ...
        WHEN x.recv_req = 'Y' THEN
          CASE
            WHEN NVL(x.qty_po,0) > NVL(x.qty_vchr,0) THEN 1 ELSE 0
          END
...
/* Open PO eligibility: open sched + active line + active distrib */
open_po_eligible AS (
...
/* Candidate POs in scope (same gates as v1 included_po, but earlier to reduce work) */
po_candidates AS (
  SELECT /*+ MATERIALIZE */ DISTINCT hc.business_unit, hc.po_id, hc.vendor_id
  FROM po_hdr_candidates hc
  JOIN open_po_eligible ope
    ON ope.business_unit = hc.business_unit
   AND ope.po_id         = hc.po_id
  WHERE EXISTS (SELECT 1 FROM ps_bh_wd_sup_1to1 wd WHERE wd.bh_wd_ps_vendor_id = hc.vendor_id)
    AND EXISTS (SELECT 1 FROM ps_bus_unit_tbl_pm bu WHERE bu.business_unit = hc.business_unit)
),

/* Service flag (unchanged semantics, but computed only for candidates) */
service_flags AS (
  SELECT /*+ MATERIALIZE */
         d.business_unit,
         d.po_id,
...
/* Amount/qty gate (same logic as v1, restricted to candidates) */
po_has_pos_amt_and_qty AS (
  SELECT so.business_unit, so.po_id
  FROM sched_open so
  JOIN service_flags pt
...
/* Final included POs (same as v1 included_po) */
included_po AS (
  SELECT /*+ MATERIALIZE */ pc.business_unit, pc.po_id
  FROM po_candidates pc
  WHERE EXISTS (
    SELECT 1 FROM po_has_pos_amt_and_qty p
    WHERE p.business_unit = pc.business_unit
      AND p.po_id         = pc.po_id
  )
),

/* Included open PO lines (same as v1; physical_nature filter remains commented) */
