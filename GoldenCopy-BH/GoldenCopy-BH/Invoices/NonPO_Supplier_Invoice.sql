/* Workday EIB – Non-PO Invoice Header – approved + unpaid + 12-month lookback
...
/* 0) Candidate vouchers (date filter WITHOUT TRUNC for index use) */
voucher_candidates AS (
  ...
  FROM ps_voucher v
  CROSS JOIN params p
  WHERE v.entry_status <> 'X' --and v.voucher_id='05312504'
...
/* 1) Exclude vouchers that have PO on ANY voucher line (restricted to candidates) */
has_po_line AS (
...
/* 2) Paid vouchers (restricted to candidates; avoids full scan of xref) */
paid_xref AS (
...
/* 3) Drive set (Approved + Unpaid + NonPO) */
voucher_base AS (
...
/* 4) ONE pass over voucher lines for these vouchers */
lines_base AS (
  SELECT /*+ MATERIALIZE */
      vl.business_unit,
      vl.voucher_id,
...
/* payment row (pymnt_cnt=1) – still ok for “On Hold” even when unpaid */
pymnt1 AS (
...
vendor_keys AS (
  SELECT /*+ MATERIALIZE */ DISTINCT vendor_setid AS setid, vendor_id
  FROM voucher_base
),
...
vendor_addr_eff AS (
  ...
  FROM (
    SELECT a.setid, a.vendor_id, a.address_seq_num,
           ROW_NUMBER() OVER (
...
vendor_pay_eff AS (
  ...
  FROM (
    SELECT p.setid, p.vendor_id, p.vndr_loc, p.eft_layout_cd,
           ROW_NUMBER() OVER (
...
vendor_loc_eff AS (
  ...
  FROM (
    SELECT l.setid, l.vendor_id, l.vndr_loc,
           ROW_NUMBER() OVER (
