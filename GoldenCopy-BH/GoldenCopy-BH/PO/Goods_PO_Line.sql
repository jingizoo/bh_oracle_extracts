...
SELECT --b.shipto_id,
   ...
   CASE WHEN TRIM(b.inv_item_id) is null THEN sc.spend_cat ELSE ' ' END
    ...
    round(b.price_po,2)                                                  AS "Unit Cost",
    ' '                                                         AS "Requested as No Charge",
    round(b.extended_amt,2)                                              AS "Extended Amount",

    ' '                                                         AS "Lot Serial Information Reference",
    ' '                                                         AS "Lot Number",
    ' '                                                         AS "Serial Number",
    case when b.buyer_id='BILLONLY' then ' ' else TO_CHAR(b.due_dt, 'YYYY-MM-DD')   end                          AS "Due Date",
