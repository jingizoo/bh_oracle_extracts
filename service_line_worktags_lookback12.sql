/* ============================================================================
   Workday EIB – Open Purchase Orders – SERVICE Line Worktags
   Update: apply same 12-month lookback + open criteria as PO conversion,
           and include ONLY service lines with remaining amount > 0.
   ============================================================================ */
...
included_service_lines AS (
  ...
  WHERE l.cancel_status <> 'X'
    ...
    AND GREATEST((NVL(s.qty_po,0) - NVL(s.qty_vchr,0)) * NVL(s.price_po,0), 0) > 0
),
unpivoted AS (
  SELECT
    d.po_id,
    ...
    ROW_NUMBER() OVER (
      ...
      ORDER BY CASE field_name WHEN 'DEPTID' THEN 1 WHEN 'PROJECT_ID' THEN 2 WHEN 'FUND_CODE' THEN 3 END
    ) AS row_num,
    value
  FROM (
    SELECT
  ...
  UNPIVOT ( value FOR field_name IN (deptid AS 'DEPTID', project_id AS 'PROJECT_ID', fund_code AS 'FUND_CODE') )
