...
hdr_candidates AS (
  SELECT /*+ MATERIALIZE */ *
  FROM po_hdr_candidates
)
,
v_paid_candidates AS (
...
paid_vouchers AS (
  SELECT /*+ MATERIALIZE LEADING(vc px) USE_NL(px) INDEX(px PSDPYMNT_VCHR_XREF) */
         DISTINCT px.business_unit, px.voucher_id
  FROM v_paid_candidates vc
...
recv_agg AS (
  SELECT /*+ MATERIALIZE */
         CAST(NULL AS VARCHAR2(5))  AS business_unit,
         CAST(NULL AS VARCHAR2(10)) AS po_id,
         CAST(NULL AS NUMBER)       AS line_nbr,
         CAST(NULL AS NUMBER)       AS sched_nbr,
         CAST(0 AS NUMBER)          AS qty_rcvd_suom,
         CAST(0 AS NUMBER)          AS merch_amt_rcvd_po
  FROM dual
  WHERE 1=0
)
,
...
service_flags AS (
  SELECT /*+ MATERIALIZE LEADING(hc d) USE_NL(d) INDEX(d PSBPO_LINE_DISTRIB) */
         d.business_unit,
         d.po_id,
