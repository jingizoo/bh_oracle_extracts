/* Performance-focused rewrite:
   - FIXED Pull_POTYPE explosion (adds rl.line_nbr = d.req_line_nbr, and aggregates to 1 row per PO)
   - Filters Pull_Comments / Pull_ProcedureInfo to OPEN POs only (driven by open_pos)
   - Replaces ROW_NUMBER() “first open sched” with KEEP (DENSE_RANK FIRST ...)
   - Uses TRUNC(SYSDATE) as as-of date so “< asof_dt + 1” means end of today
   - Removes invoice/receiving status CTEs (you were outputting NULL anyway)
*/
...
/* Header candidates (cheap filter first) */
hdr_candidates AS (
  SELECT /*+ MATERIALIZE */
         h.business_unit,
         h.po_id,
...
/* Receipts aggregated by PO schedule as-of (restricted to header candidates) */
recv_agg AS (
...
/* ===== Comments (restricted to open_pos) ===== */
