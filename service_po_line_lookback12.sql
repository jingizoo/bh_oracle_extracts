/* ============================================================================
   Workday EIB – Open Purchase Orders – SERVICE Line Replacement Data
   Update: apply 12-month lookback per conversion slide (PO / receipt / invoice activity),
           and include ONLY service lines with remaining (unvouchered) amount > 0.
   ============================================================================ */
...
base AS (
  SELECT
      ...
      s.qty_vchr,
      s.price_po,
      NVL(s.merchandise_amt, (s.qty_po * s.price_po)) AS po_line_amt,
      GREATEST((NVL(s.qty_po,0) - NVL(s.qty_vchr,0)) * NVL(s.price_po,0), 0) AS remaining_amt,
...
SELECT
  ...
  b.remaining_amt                                             AS "Extended Amount",
